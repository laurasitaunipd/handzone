...
![](overfitting.png){fig-align="center" width="369"}

# Dalla predizione alla causazione

$y_i = \beta_0 + \beta_1T_i + \beta_2x_i + e_i$

![](pigeon.jpeg){fig-align="center" width="393"}
...
# Think before you regress: DAG 1
...
-   Nei DAG la freccia è usata quando si ipotizza una relazione *causale* ad es. da X a Y (X -\>Y). Si può anche ipotizzare, ma va giustificata, l'assenza di una relazione, oppure si può ipotizzare una relazione bidirezionale (X \<-\> Y), che però è equivalente a ipotizzare la presenza du una variable non osservata - o latente - che causa entrambe (X \<- U -\> Y).
...
Sia nelle catene che nelle forchette, controllare per Z **blocca** il percorso (*path*) introducendo un'indipendenza condizionata tra X e Y: $X \perp\!\!\!\perp Y \mid Z$

Nei *collider*, controllare per Z **apre** il percorso (*path*) introducendo una dipendenza condizionata tra X e Y: $X \not \perp\!\!\! \not \perp Y \mid Z$

La *d*-separation di due variabili si ha quando, attraverso una covariata, si blocca ogni *path* tra loro è bloccata. Viceversa, si ha la *d*- connection. Questi concetti sono importanti per DAG con più variabili. Ad esempio, controllare per un discendente di un collisore rischia di creare una *d*-connection tra variabili che prima erano *d*-separated.
...
# Possibili bias nella stima

![Del Giudice e Gangestead, 2021, SM](DelGiudice_1.png){fig-align="left" width="400"}
...
-   Studio su donne con iperplasia surrenale congenita (CAH) e sociosessualità promiscua
    ...
    -   Per quli variabili matchare i controlli? Primi candidati: età, livello di istruzione, orientamento politico, religiosità, status relazionale...ma ha senso questa insalata causale?
...
<https://dagitty.net/dags.html?id=rrZZzgPV>
