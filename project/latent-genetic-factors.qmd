...
How do we identify the broad (latent) factors?

- approach by Grotzinger et al., 2022: genomic structural equation modeling (genomic SEM)
- proposed approach: Non-negative matrix factorization (NMF), inspired by Landy et al., 2025
...
identify four broad factors that index shared genetic liability within
and across constellations of disorders.
...
the full observed data matrix $\mathbf{Y} \in \mathbb{R}^{D \times N}$ of $N$ individuals $i$
</center>
...
NMF used in two algorithms using `causalLFO` package
